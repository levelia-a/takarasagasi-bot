...
作成するテーブルは `settings`・`statistics`・`user_progress`・`admin_logs` の4つです。`user_progress` はユーザーごとの初級・中級の探索回数を保存します。難易度の解放状態そのものは保存せず、現在の探索回数と現在の解放条件を比較して判定します。
...
- 初期設定は初級 `1000 / 60% / 5回`、中級 `5000 / 50% / 7回`、上級 `10000 / 40% / 10回` です。初級を10回探索すると中級、中級を20回探索すると上級に挑戦できます。成功・失敗に関係なく通常プレイの探索判定ごとに1回加算し、テストモードは進捗に含めません。解放判定は保存済みの探索回数と現在の管理者設定を毎回比較するため、必要回数を引き上げて探索回数が条件未満になった場合は再び挑戦できなくなります。
