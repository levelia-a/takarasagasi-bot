...
作成するテーブルは `settings`・`statistics`・`user_progress`・`admin_logs` の4つです。`user_progress` はユーザーごとの探索回数と、一度獲得した難易度解放状態を保存します。
...
- 初期設定は初級 `1000 / 60% / 5回`、中級 `5000 / 50% / 7回`、上級 `10000 / 40% / 10回` です。初級を10回探索すると中級、中級を20回探索すると上級が解放されます。成功・失敗に関係なく通常プレイの探索判定ごとに1回加算し、テストモードは進捗に含めません。一度解放した難易度は、後から管理者が必要回数を引き上げても再ロックされません。
