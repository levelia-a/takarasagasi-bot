...
地図の銅・銀・金の色は開始画面で表示します。探索結果のEmbed色は、地図とは別の探索色抽選で決まります。
地図の効果はその宝探しの全探索判定に有効で、終了後に次のゲームへ持ち越しません。
開始後のDB保存再試行・ボタン操作では地図を再抽選しません。

出現率の仮設定は通常95.7%、銅3%、銀1%、金0.3%。成功率への加算は通常0、銅5、銀10、金20ポイント、
補正後は100%が上限です。管理パネルの「地図・レア設定」で変更できます。難易度別の元の成功率は既存設定を使います。
テストモードでも地図を表示しますが、「必ず成功」「必ず失敗」の判定が優先されます。
地図はセッション内だけに保持し、DB構造の変更はありません。

### 探索色と宝物レア度

探索判定ごとに青・緑・赤を独立に抽選します。色の効果はその判定1回分だけです。
青→緑→赤の順でレア以上の宝物が出やすくなります。成功率そのものは地図の効果だけで決まります。
失敗時はどの色でも宝物を獲得できません。撤退時は最後の探索色を表示するだけで再抽選しません。
DB保存の再試行でも同じ色・宝物を維持します。

仮設定は青80%・緑17%・赤3%。青は元の宝物確率、緑はレア以上の抽選重みを2倍、赤は4倍にして、
全体を100%に換算します（実際の当選確率がそのまま2倍・4倍になる意味ではありません）。
ノーマル90%・レア10%なら、補正後のレア率は青10%、緑約18.2%、赤約30.8%です。
レア・エピック・レジェンドの間の確率比は維持します。元の確率0の宝物は出現しません。
全宝物がノーマル、または全宝物がレア以上の場合は比率が変わらず、補正による変化はありません。
管理パネルの「地図・レア設定」で色の出現率・倍率を変更できます。DB未登録時の初期値は`consts/balance.py`です。

初期バランス案は宝物全体でノーマル75%・レア20%・エピック4%・レジェンド1%です。
各10種類の配分例はノーマル5種類を各15%、レア3種類を8%・7%・5%、エピック4%、レジェンド1%。
この配分なら成功時のレア以上は青25%・緑40%・赤約57.14%、色を平均して約28.51%になります。
レジェンドは青1%・緑1.6%・赤約2.29%、色を平均して約1.14%です。
...
レア・エピック・レジェンドの重みを同じ倍率で増やし、既存の青・緑・赤の倍率と掛け合わせます。
当選率そのものが倍になるわけではありません。元が0%の宝物は0%のままです。
例：基本レア以上25%の場合、青で6人以上・イベントなしなら約34.8%、宝物庫なら約44.4%です。
