...
「協力探索イベント」は各自の宝探し開始時に1回抽選します。当選すると追加で探索＋1回、
レア以上の重み1.25倍を、その宝探し全体へ適用します（6人以上なら合計＋4回・2倍）。
...
例：基本レア以上25%の場合、青で6人以上・イベントなしなら約34.8%、イベントありなら40%です。
成功率、失敗時の報酬喪失、撤退、ランキング・進捗の保存方法は従来どおりです。

`/takara_admin` → **VC協力設定** でON/OFF・3段階の必要人数・追加探索回数・重み倍率・
イベント発生率・当選時の追加効果を編集できます。保存後は次の宝探しから適用されます。
