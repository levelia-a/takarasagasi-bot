...
地図の特別な色は開始画面と最初の探索結果だけに表示し、「さらに奥へ」で2回目の判定に進むと通常色へ戻ります。
色が戻った後も成功率の補正は終了まで継続します。
地図の効果はその宝探しの全探索判定に有効で、終了後に次のゲームへ持ち越しません。
開始後のDB保存再試行・ボタン操作では地図を再抽選しません。

出現率の仮設定は通常95.7%、銅3%、銀1%、金0.3%。成功率への加算は通常0、銅5、銀10、金20ポイント、
補正後は100%が上限です。`consts/maps.py` で設定します。難易度別の元の成功率は既存設定を使います。
...
各宝物には次の4項目を指定し、対応難易度は所属する配列で決まります。
